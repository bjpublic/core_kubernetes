<!--
  Title: 핵심만 콕 쿠버네티스
  Description: 쿠버네티스를 처음 접하시는 분을 위해 준비하였습니다. 컨테이너, 쿠버네티스 기술에 대해 잘 모르더라도 이 책의 예제를 따라하다보면 어느새 쿠버네티스의 매력에 푹 빠질 것입니다. 쿠버네티스 입문서로 시작하기에 최고의 선택, 여러분도 쿠버네티스라는 거인의 어깨 위에 올라서서 클라우드 네이티브가 꿈꾸는 세상을 바라보시기 바랍니다.
  Author: 유홍근 (커피고래)
-->
# 핵심만 콕! 쿠버네티스

- 부제: 쿠버네티스의 핵심을 실습하고 이해하는
- 저자: 유홍근 ([커피고래](https://coffeewhale.com))
- 출간: 2020년 9월 18일
- 정가: 32,000원
- 페이지: 504
...
1. [도커 기초](chapters/01)
    ...
    - 1.5 도커 실행 고급
    - 1.6 마치며
2. [쿠버네티스 소개](chapters/02)
    - 2.1 쿠버네티스란?
    - 2.2 쿠버네티스의 기본 개념
    - 2.3 아키텍처
    - 2.4 장점
    - 2.5 마치며
3. [쿠버네티스 설치](chapters/03)
    - 3.1 k3s 소개
    - 3.2 k3s 설치하기
    - 3.3 마치며
4. [쿠버네티스 첫 만남](chapters/04)
    - 4.1 기본 명령
    - 4.2 고급 명령
    - 4.3 마치며
5. [Pod 살펴보기](chapters/05)
    - 5.1 Pod 소개
    - 5.2 라벨링 시스템
    - 5.3 실행 명령 및 파라미터 지정
    - 5.4 환경변수 설정
    ...
    - 5.12 메타데이터 전달
    - 5.13 마치며
6. [쿠버네티스 네트워킹](chapters/06)
    - 6.1 Service 소개
    - 6.2 Service 종류
    - 6.3 네트워크 모델
    - 6.4 마치며
7. [쿠버네티스 컨트롤러](chapters/07)
    - 7.1 컨트롤러란?
    - 7.2 ReplicaSet
    - 7.3 Deployment
    - 7.4 StatefulSet
    ...
    - 7.6 Job & CronJob
    - 7.7 마치며
8. [helm 패키지 매니저](chapters/08)
    - 8.1 helm이란?
    - 8.2 원격 리파지토리(repository)
    - 8.3 외부 chart 설치(WordPress)
    - 8.4 마치며
9. [Ingress 리소스](chapters/09)
    - 9.1 Ingress란?
    - 9.2 Ingress 기본 사용법
    - 9.3 Basic Auth 설정
    - 9.4 TLS 설정
